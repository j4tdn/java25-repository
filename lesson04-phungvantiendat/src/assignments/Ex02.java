...
public class Ex02 {
	...
	private static Employee[] mockData() {
        Director director = new Director("Nguyen Van An", "1975-02-15", 2.5, 4.0);
	...
	private static void staffStat(Employee[] employees) {
		DepartmentHead[] heads = new DepartmentHead[2];
	    int[] count = new int[2];
	    
	    int index = 0;
	    for (Employee e : employees) {
	        if (e instanceof DepartmentHead) {
	            heads[index++] = (DepartmentHead) e;
	        }
	    }
	    ...
	    System.out.println("\n=== Statistics ===");
	    for (int i = 0; i < heads.length; i++) {
	        System.out.println("- " + heads[i].getFullName() + " (" + heads[i].getDepartment() + "): "
	                + count[i] + " employees");
	    }
	}
}
