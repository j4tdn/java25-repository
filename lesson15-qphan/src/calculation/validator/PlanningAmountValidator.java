...
import calculation.exception.MinimumAmountException;
...
public class PlanningAmountValidator implements Validator {
	...
	public void validate() throws CalculationException {
		if (planningAmount < requiredMinPlanningAmount) {
			throw new MinimumAmountException("Planning Amount(" + planningAmount + ") "
					+ "is less then required min Planning Amount(" + requiredMinPlanningAmount + ")");
		}
		
		log(">>> Planning Amount (%s) is greater than "
				+ "required min Planning Amount(%s)", planningAmount, requiredMinPlanningAmount);
	}
