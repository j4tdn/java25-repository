	...
	<div class="content">
		...
		<div class="mt-4">
			<div class="search w-10 px-3 d-flex justify-content-between">
				<form class="w-25"
					action="${pageContext.request.contextPath}/ChartRevenue"
					method="POST">
				...
				<a href="
					${pageContext.request.contextPath}/downloadFileRevenue?yearex=${year}">
			...
			<c:if test="${not empty revenueData}">
				...
				function drawChart() {
					var data = google.visualization.arrayToDataTable([
						['Tháng', 'Doanh thu'],
						<c:forEach items="${revenueData}" var="item" varStatus="loop">
						    ['Tháng ${item[0]}', ${item[1]>=0 ? item[1]:0}] <c:if test="${!loop.last}">,</c:if>
						</c:forEach>
					]);
					//var maxValue = Math.max(...data.map(row => row[1])); // Lấy giá trị lớn nhất của doanh thu
